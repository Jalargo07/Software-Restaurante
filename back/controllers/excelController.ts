...
import { Venta, DetalleVenta, Producto, Compra, DetalleCompra, Proveedor, Auditoria } from '../models';
import { Op } from 'sequelize';
import { scopeTenant } from '../utils/tenantScope';

export const reporteVentasExcel = async (req: Request, res: Response) => {
  try {
    ...
    for (const venta of ventas) {
      const productosLista = (venta.DetalleVentas || [])
        .map((d: any) => `${d.Producto?.nombre || 'N/A'} x${d.cantidad}`)
        .join(', ');
    ...
    return res.status(500).json({ error: 'Error al exportar reporte de ventas' });
  }
};

export const reporteComprasExcel = async (req: Request, res: Response) => {
  try {
    const { fechaDesde, fechaHasta } = req.query;
    ...
    if (fechaDesde || fechaHasta) {
      where.fecha = {};
      if (fechaDesde) where.fecha[Op.gte] = new Date(fechaDesde as string);
      if (fechaHasta) {
        const hasta = new Date(fechaHasta as string);
        hasta.setHours(23, 59, 59, 999);
    ...
    return res.status(500).json({ error: 'Error al exportar reporte de compras' });
  }
};

export const reporteAuditoriaExcel = async (req: Request, res: Response) => {
  try {
    ...
    const logs: any = await Auditoria.findAll({
      where: scopeTenant(where, req.tenantId!),
      order: [['createdAt', 'DESC']],
    });
    ...
    return res.status(500).json({ error: 'Error al exportar reporte de auditoría' });
  }
};
